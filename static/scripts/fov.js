...
// courtesy of: http://roguebasin.roguelikedevelopment.org/index.php/Permissive_Field_of_View_in_Javascript
...
Arc.prototype.copy = function() {
  ...
  for (i in this.steepbumps) {
    if (this.steepbumps.hasOwnProperty(i)) {
      c.steepbumps.push(this.steepbumps[i].copy());
    }
  }
  for (i in this.shallowbumps) {
    if (this.shallowbumps.hasOwnProperty(i)) {
